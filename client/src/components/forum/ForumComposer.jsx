...
export default function ForumComposer({ onClose, onPublish }) {
  const [form, setForm] = useState({ title: "", summary: "", content_html: "", visibility: "public" });
  ...
  return (
    <Modal title="Nuevo post" onClose={onClose}>
      <form className="settings-section" onSubmit={submit}>
        ...
        <Button variant="primary" disabled={loading}>{loading ? "Publicando..." : "Publicar"}</Button>
      </form>
    </Modal>
  );
}
