...
import { createPost as createForumPost, getPosts, toggleLike, toggleSave } from "../../services/forumService";
...
export default function ForumFeed({ toast }) {
  ...
  async function load(offset = 0, append = false) {
    const data = await getPosts({ filter, q: debouncedQ, offset, limit: 20 });
  ...
  async function createPost(payload) {
    const data = await createForumPost(payload);
