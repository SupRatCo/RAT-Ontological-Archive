(function () {
  ...
  function renderTab(tab, settings, projects) {
    ...
    if (tab === "server") return `<div class="panel flat"><h3>Servidor</h3><p class="meta">API: ${UI.escape(window.ROA.Api.baseUrl || "Sin configurar")}</p><button class="action" type="button" data-action="test-server">Test</button><p id="serverStatus" class="meta"></p></div>`;
  ...
  async function testServer() {
    ...
    try {
      ...
      if (node) node.textContent = `Conectado: ${health.mode || "api"}`;
    } catch (error) {
  ...
  function playSound(name) {
    ...
    if (volume) window.ROA.lastSoundRequest = { name, volume, at: Date.now() };
  }

  window.ROA.Settings = { applySettings, openSettings, exportAll, resetAllData, importBackupFile, playSound, themeLabels, testServer };
